#!/bin/bash
# Generate per-replica ACCESS_KEY values in PseudoCo Assistant's established four-word
...
#
# WHY THIS EXISTS: PseudoCo Assistant has no key generator. backend/config.py declares
...
ensure_keyfile() {
  if [ ! -f "$KEYFILE" ]; then
    umask 077
    {
      echo "# PseudoCo Assistant per-replica ACCESS_KEY values."
