#!/bin/bash
# Regression test for the PseudoCo Assistant -> OpenTelemetry -> Splunk Observability Cloud
...
if [ -z "${APITOK:-}" ]; then
  ...
  if python3 tests/observability/check_o11y_metadata.py "$REALM" "$APITOK" pseudoco-assistant pseudoco-assistant-local; then
