...
# alternative Mode-C gateway for the agentic-risk demo, with PseudoCo Assistant's
# governance seat baked in:
#   - the sandbox image is NemoClaw's pinned OpenClaw sandbox + the
#     pseudoco-assistant-toolguard plugin (nemoclaw/Dockerfile), so every agent tool call
...
#   - a network-policy preset lets the sandbox reach ONLY PseudoCo Assistant's guard
#     endpoint (nemoclaw/policies/pseudoco-assistant-guard.yaml) — everything else stays
...
#   ./run-nemoclaw.sh --host 10.0.1.5       # PseudoCo Assistant reachable at that private IP from the sandbox
...
HOST=""                          # how the SANDBOX reaches PseudoCo Assistant; default: this host's private IP
FOREGROUND=false
FORWARDER=true
...
# ---------- PseudoCo Assistant reachability ----------
if [ -z "$HOST" ]; then
  # The sandbox must reach PseudoCo Assistant at THIS HOST'S PRIVATE IP. NemoClaw's
...
curl -s --max-time 3 "$LOCAL_GUARD/health" >/dev/null \
  || warn "PseudoCo Assistant is not answering at $LOCAL_GUARD — the tool guard is FAIL-CLOSED, so every sandbox tool call is denied until ./run.sh is up"

# ---------- NemoClaw CLI ----------
if ! command -v nemoclaw >/dev/null; then
  log "installing NemoClaw (ref $INSTALL_REF)"
  [ "$PROVIDER" = build ] && [ -z "${NVIDIA_INFERENCE_API_KEY:-}" ] \
    && die "NVIDIA_INFERENCE_API_KEY (nvapi-…) is required for --provider build (the sandbox's inference; PseudoCo Assistant's own provider stays local)"
...
CTX=nemoclaw/plugins/pseudoco-assistant-toolguard
cleanup() { rm -rf nemoclaw/plugins; }
trap cleanup EXIT
mkdir -p "$CTX"
git archive HEAD:openclaw/plugins/pseudoco-assistant-toolguard | tar -x -C "$CTX"
[ -f "$CTX/index.js" ] || die "plugin not found in git (HEAD:openclaw/plugins/pseudoco-assistant-toolguard)"
...
timeout 90 openshell sandbox exec --name "$SANDBOX" -- env \
    PSEUDOCO_ASSISTANT_GUARD_URL="$GUARD" PSEUDOCO_ASSISTANT_ACCESS_KEY="$ACCESS_KEY" \
    node -e '
...
# ---------- network policy: only PseudoCo Assistant's guard endpoint ----------
log "applying the pseudoco-assistant-guard network policy (host $HOST)"
POLICY_TMP=$(mktemp "$STATE_DIR/pseudoco-assistant-guard.XXXXXX.yaml")
sed "s/__PSEUDOCO_ASSISTANT_HOST__/$HOST/g" nemoclaw/policies/pseudoco-assistant-guard.yaml > "$POLICY_TMP"
