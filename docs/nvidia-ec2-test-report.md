# NVIDIA stack test report — PseudoCo Assistant 4.2.1 on a g5.xlarge EC2 box (+ Mac baseline)
...
| Ollama chat turn, default blueprint | **PASS** | `recommendation`, 581 chars; governance `provider=ollama model=mistral-nemo:12b blueprint=pseudoco_multi_agent policy_action=allow` |
...
| NVIDIA AI Virtual Assistant blueprint on Ollama (per-request `blueprint`) | **PASS** | record-lookup turn 642 chars, knowledge-retrieval turn 717 chars; governance `blueprint=nvidia_virtual_assistant workflow=pseudoco_nvidia_virtual_assistant`; `GET /api/analytics/sessions` lists the session; server default unchanged |
...
| # | Check | Result | Evidence |
|---|---|---|---|
| A | A10G present; Ollama holds nothing; `pseudoco-assistant-nim` container + all six units active; `/v1/health/ready` 200; `/v1/models` serves the Nemotron id; direct completion with `enable_thinking=false` | **PASS** 10/10 | direct NIM: 23 prompt + 200 completion tokens in 8.4 s; VRAM 20.1 GB used |
...
| D | NemoClaw policy layer + sandbox runtime: `verify_nemoclaw_observability.sh` Tier 0 (policy, Dockerfile, preset, forwarder, unit suite, plugin selftest), Tier 1 (toggle ON blocks an unlisted egress via the policy layer; a forwarded OCSF denial becomes a `nemoclaw_guardrails` governance event), **Tier 2 live sandbox** (answers status, `pseudoco-assistant-toolguard` enabled inside, the sandbox reaches PseudoCo Assistant's guard through the applied `pseudoco-assistant-guard` policy) | **PASS** 13/13 (after defects 8, 9, 11 and the `start`-re-provision fix) | sandbox guard URL `http://172.31.7.215:8001`; `Applied preset: pseudoco-assistant-guard`; `runtime_supported` ON; `toolguard/nemoclaw` shows `pseudoco-assistant-guard` among the loaded network policies. First passes failed on exactly the defects listed; one read taken during the script's own `nemoclaw gateway restart` was transiently 11/13 and was 13/13 three minutes later |
...
| F | Telemetry under `deployment.environment=pseudoco-assistant-ec2-1` | **PASS (spans)** / **known gap (metrics)** | box collector counters: 510 spans → Splunk APM, 147 → Splunk Agent Observability, 10 503 metric points, no `send_failed` counters; `check_o11y_metadata.py … pseudoco-assistant-ec2-1` fails on the gen_ai metric MTS exactly as the Mac does (org-wide metric cap since 2026-07-29, unrelated to 4.2) |
...
Procedure (matrix section H): stop `pseudoco-assistant-nim`, `PUT /api/settings/ai-provider {"provider":"ollama"}`, chat turns
on `mistral-nemo:12b` and on the poisoned model, the synthetic-PII injection with Cisco AI Defense review, then
back to `provider=nvidia` and `pseudoco-assistant-nim`.
...
| Back to `provider=nvidia`, NIM restarted and ready | **PASS** | NIM ready after the reload (300 s to `/v1/health/ready` after `systemctl start pseudoco-assistant-nim` with all Ollama models unloaded first; next turn 19.8 s); governance `provider=nvidia` on the next turn |
...
| Check | Result | Evidence |
|---|---|---|
| `sudo systemctl reboot`; everything must return unattended | **PASS** in 380 s | app `/health` 200 and the public URL 200 at +30 s; collector + tunnel active at +30 s; `pseudoco-assistant-nemoclaw` (oneshot) re-onboarded the sandbox and the forwarder came up at +53 s; the NIM reached `/v1/health/ready` at +343 s (weights + profiling, no restart loop); sandbox phase Ready |
